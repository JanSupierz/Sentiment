...
\begin{enumerate}
   ...
   \item \textbf{Tag removal} replaces any substrings within the HTML tags \code{<>} with a space.
	...
	\item \textbf{Contraction expansion} is implemented using \code{con\-tra\-ctions.fix} function to expand English contractions (e.g., \literal{don't} $\rightarrow$ \literal{do not}).
	...
	\item \textbf{POS-filtered tokenization} replaces the standard stop-word removal (Sec.~\ref{sec:stop-word-removal}); the expanded text (\code{tokens\_\-lower}) is filtered based on the POS tags; the result is stored as \code{tokens\_filtered}.
...
\label{eq:accuracy}
\end{equation}
\ksremark{Trzeba „Accuracy” zastąpić jakimś symbolem ($A$?) we wzorze \eqref{eq:accuracy}.}
...
\begin{equation}
	...
	\label{eq:macro:f1}
\end{equation}
\ksremark{Wzór \eqref{eq:macro:f1}: tutaj też trzeba wymyślić jakiś symbol.}
...
The $\mcnemarp$-value is the probability that a $\mcnemarchi_1$ random variable \ksremark{Co oznacza 1 w  $\mcnemarchi_1$?} would be at least as large as the observed value $\mcnemarchi_{\text{obs}}$.
