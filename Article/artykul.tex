...
\hyphenation{in-ter-pre-ta-bi-li-ty}
...
\newcommand{\stepnum}[1]{\tikz[baseline=(char.base)]{\node[circle, draw=black, fill=white, inner sep=1pt, minimum size=3.5mm, font=\scriptsize\bfseries] (char) {#1};}}
...
\newcommand{\negative}{\eta}
\newcommand{\smoothedprob}[2]{s_{#1}(#2)}
\newcommand{\Count}[2]{C_{#1}(#2)}
...
\newcommand{\mcnemarstat}{q}
\newcommand{\mcnemarstatrand}{Q}
...
\author[1]{Jan Supierz}[orcid=0009-0001-8915-7770]
\cormark[1]
%\fnmark[1]
\ead{jan.supierz02@gmail.com}
...
\begin{abstract}
	The explosion of user-generated content on the internet has made sentiment analysis a critical tool for organizations seeking to understand public opinion and drive data-driven decision-making. However, processing these massive text streams in real time poses severe computational challenges. 
	Sentiment analysis models face a persistent trade-off between the high accuracy of deep learning and the computational efficiency required for practical deployment. 
	This paper investigates how granular computing techniques can bridge this gap. 
	The main contribution of this work is an optimized lightweight cascade model, where a linear support vector machine handles the majority of reviews with high confidence, while a specialist Transformer-based model resolves only the remaining uncertain cases. 
	The proposed method is evaluated on the IMDb movie review dataset, achieving a macro $F_1$-score of approximately $92.3\%$. 
	This architecture significantly outperforms both standalone baselines while delegating only $15\%$--$20\%$ of the test instances to the deep-learning specialist. 
	These results demonstrate that granular vocabulary reduction combined with model-level deferral can successfully balance accuracy and efficiency in sentiment analysis systems.
\end{abstract}
...
The rapid growth of user-generated content on social media platforms, review portals, discussion forums, and e-commerce websites has created an unprecedented demand for automated methods capable of extracting opinions from textual data. Sentiment Analysis (SA), also referred to as opinion mining, addresses this challenge by computationally identifying and interpreting opinions, emotions, attitudes, and evaluations expressed in natural language~\cite{Liu2012sentiment}. The field has become an essential component of modern decision-support systems and is widely applied in customer feedback analysis, brand monitoring, recommendation systems, market intelligence, political trend analysis, and public opinion monitoring~\cite{behdenna2018document, sharma2025review}.
...
\end{equation}
%
where $e_i$ denotes the $i$-th entity, $a_{ij}$ represents the $j$-th aspect of the $i$-th entity, $h_k$ is the opinion holder, $t_l$ stands for the time of expression, and $s_{ijkl}$ is the sentiment associated with the $j$-th aspect of the $i$-th entity expressed by the $k$-th opinion holder at time $t_l$. According to this formulation, the ultimate goal of sentiment analysis is to extract all opinion quintuples present in a given document~\cite{Liu2012sentiment}.

Depending on the level of granularity, sentiment analysis can be performed at the document, sentence, or aspect level~\cite{azzeddine2018sentiment}. Document-level sentiment analysis aims to determine the overall polarity of a text and remains the most widely studied setting~\cite{behdenna2018document, ravi2015survey}. Sentence-level approaches provide finer granularity by classifying individual subjective statements~\cite{CHATURVEDI201865}, while Aspect-Based Sentiment Analysis (ABSA) identifies sentiments directed toward specific attributes of an entity, thereby enabling a more detailed understanding of user opinions~\cite{yadav2020sentiment, sharma2025review}. Beyond polarity classification, contemporary research has expanded toward related tasks such as emotion detection, multilingual sentiment analysis, multimodal sentiment analysis, cross-domain adaptation, and opinion spam detection~\cite{sharma2025review}.

Despite decades of progress, sentiment analysis remains challenging due to several linguistic and contextual phenomena. The polarity of sentiment-bearing words often depends on context, making simple lexical approaches insufficient~\cite{behdenna2018document}. Additional difficulties arise from sarcasm, irony, negation, implicit opinions, domain-dependent vocabulary, and the continuous emergence of new linguistic expressions, particularly in social media environments~\cite{behdenna2018document, islam2024challenges, sharma2025review}. These challenges motivate the development of increasingly sophisticated models capable of capturing both semantic and contextual information.

Traditional sentiment analysis methods can be broadly divided into lexicon-based approaches and supervised machine learning techniques~\cite{azzeddine2018sentiment, sharma2025review}. More recently, deep learning architectures---such as Convolutional Neural Networks (CNNs)~\cite{id:Xie2023Convolutional}, Recurrent Neural Networks (RNNs)~\cite{id:Nursal2025Battle}, Long Short-Term Memory (LSTM) networks~\cite{id:Kilic2026Hybrid,id:Tan2022RoBERTa}, and Transformer-based models---have become dominant due to their ability to learn complex semantic representations directly from data~\cite{ain2017sentiment, sharma2025review}. Although these models achieve state-of-the-art performance, they require substantial computational resources and large amounts of labeled data~\cite{Li2017deep, strubell2019energy}.
...
The remainder of this paper is organized as follows. Section~\ref{sec:related_work} reviews existing sentiment analysis approaches and granular computing techniques relevant to text classification. Section~\ref{sec:our-approach} presents the proposed granular cascade framework. The experimental setup and results are discussed in subsequent sections, followed by conclusions and directions for future research.
...
The availability of annotated corpora led to the widespread adoption of supervised machine learning techniques for sentiment classification~\cite{azzeddine2018sentiment}. Unlike lexicon-based systems, these approaches learn predictive models directly from labeled examples, allowing them to capture more complex relationships between linguistic features and sentiment labels.

Traditional machine learning methods typically represent documents using Bag-of-Words (BoW), $n$-gram, or Term Frequency-Inverse Document Frequency (TF-IDF) features~\cite{behdenna2018document}. Additional syntactic and semantic information, such as Part-of-Speech (POS) tags, dependency relations, sentiment-bearing expressions, and negation indicators, is often incorporated through feature engineering~\cite{azzeddine2018sentiment}. The resulting feature vectors are then processed by classifiers such as Naïve Bayes (NB), Logistic Regression (LR), and Support Vector Machines (SVMs).
...
The work is further guided by the following research questions:

\subparagraph{RQ1:} \emph{To what extent can feature-level granulation strategies improve the sentiment analysis pipeline?}

\subparagraph{RQ2:} \emph{How can a three-way decision cascade architecture combine the efficiency of lightweight classifiers with the semantic understanding of Transformer-based models?}

\jsremark{Dodałem sekcję o RQ}
...
\jsremark{dodałem numerację}
The proposed system operates at three levels (Fig.~\ref{fig:pipeline}).
\paragraph{Data Preparation} First, the raw IMDb dataset \stepnum{1} undergoes preprocessing \stepnum{2} to ensure consistency and create a basis for the experiments.
\paragraph{Feature Engineering} Next, Granular Computing (GrC) approaches---custom vectorization \stepnum{3} and optional granulation \stepnum{4}---are utilized to find the most informative features, and the most robust representations and models.
\paragraph{Cascade Classifier} Finally, a cascade ensemble based on the 3WD paradigm is introduced: a traditional machine learning base model \stepnum{5} provides rapid predictions for clear-cut cases, while a deep learning model \stepnum{6} is reserved for instances where confidence of the base model is low. The output of either model becomes the sentiment prediction \stepnum{7}.
\begin{figure*}[htbp]
	\centering
	\begin{tikzpicture}[node distance=0.6cm and 1.2cm]
		
		% Row 1
		\node[datanode] (imdb) {IMDb Dataset \stepnum{1}};
		
		\node[stepnode, right=of imdb] (prep)
		{Data Preprocessing \stepnum{2}\\
			\scriptsize cleaning, \\
			\scriptsize tokenization };
		
		\node[stepnode, right=2cm of prep] (vector)
		{Custom Vectorization \stepnum{3}\\
			\scriptsize $n$-gram range, \\
			\scriptsize $\zscore$-score pruning};
		
		% row 2
		\node[stepnode, below=1.2cm of vector, dashed] (gran)
		{Granulation \stepnum{4}\\
			\scriptsize \code{FAISS} $K$-Means,\\
			\scriptsize \code{VADER} weight \\
			\scriptsize (Optional)};
		
		\node[stepnode, left=3cm of gran] (base)
		{Base Model \stepnum{5}\\
			\scriptsize SVM or \logreg{}};
		
		\node[stepnode, left=1.8cm of base] (specialist)
		{Specialist \stepnum{6}\\
			\scriptsize BERT (or SVM)};
		
		% row 3
		\node[finalnode, below=1.2cm of base] (pred)
		{Sentiment Prediction \stepnum{7}};
		...
		node[midway, above, font=\scriptsize] {if uncertain};
		
		% Output
		\draw[arrow] (base.south) -- (pred.north)
		node[midway, right, font=\scriptsize] {certain};
		...
		\begin{scope}[on background layer]
			...
			\node[draw=datacolor!50, fill=datacolor!5, rounded corners=6pt, inner sep=15pt,
			fit=(imdb) (prep)] (dpBox) {};
			\node[font=\sffamily\bfseries\scriptsize, anchor=north west]
			at (dpBox.north west) {Data Preparation};
			
			% 2. Feature Engineering Box
			\node[draw=gray!30, fill=gray!5, rounded corners=6pt, inner sep=15pt,
			fit=(vector) (gran)] (feBox) {};
			\node[font=\sffamily\bfseries\scriptsize, anchor=north west]
			at (feBox.north west) {Feature Engineering};
			
			% 3. Cascade Classifier Box
			\node[draw=blue!20, fill=blue!2, rounded corners=6pt, inner sep=15pt,
			fit=(base) (specialist) (pred)] (ccBox) {};
			\node[font=\sffamily\bfseries\scriptsize, anchor=north west]
			at (ccBox.north west) {Cascade Classifier};
			
		\end{scope}
		
	\end{tikzpicture}
	\caption{High-level overview of the proposed granular cascade ensemble sentiment analysis pipeline. Numbers 1--7 denote the components described in the text.}
	\label{fig:pipeline}
\end{figure*}
...
\begin{enumerate}
	\item {Unicode normalization} converts the reviews to Normalization Form Compatibility Composition (NFKC) form to ensure a consistent character representation.
...
During the POS-filtering step, the dimensionality of the $n$-gram feature space can be reduced by removing words that carry little sentiment value---commonly termed stop-words.

The English stop-word list provided by scikit-learn's \code{CountVectorizer} contains the most common English words, however, the presence of words such as \literal{well} may be informative for sentiment analysis (e.g., \literal{well filmed}). In this work, it has been decided to bypass this approach for stop-word removal.

Instead, stop-word filtering is kept to a minimum by utilizing a custom POS-based approach. This approach relies on the tags assigned by the spaCy library: coarse-grained Universal POS (UPOS) tags\footnote{\url{https://universaldependencies.org/u/pos/}} and fine-grained Penn Treebank tags\footnote{\url{https://www.ling.upenn.edu/courses/Fall_2003/ling001/penn_treebank_pos.html}}. The POS filtering removes tokens that satisfy any of three specific criteria:
...
To evaluate the impact of vocabulary representation on model performance, the tokenized text is transformed into discrete feature spaces using combinations of unigrams, bigrams, and trigrams. The higher-order $n$-grams naturally introduce massive, highly sparse feature spaces. To mitigate the curse of dimensionality, an attempt is made to employ a few dimensionality reduction techniques: custom vectorization, statistical pruning and semantic clustering.

\subsubsection{Custom Vectorization} \label{sec:custom_vec}

By default, scikit-learn's \code{CountVectorizer} removes terms listed in the \code{stop\_words} parameter, generates $n$-grams, and then prunes the vocabulary according to the \code{min\_df} and \code{max\_df} thresholds. To evaluate the various token representations produced during preprocessing, it is essential to preserve their exact structures. Therefore, the default tokenization behavior is overridden.

At this stage, each document is a list of tokens. The lists are joined into single, space-separated strings, and a custom \code{space\_tokenizer} is supplied to the vectorizer. This tokenizer bypasses the default regular-expression-based tokenizer and splits text strictly on spaces. The vectorizer is additionally configured with \code{lowercase=False} (lowercasing was already performed during preprocessing) and the desired \code{ngram\_range} (e.g., $(1,3)$). This customized pipeline ensures that scikit-learn's frequency-based pruning is applied directly to the finalized $n$-grams, effectively reducing dimensionality and promoting model generalization. For this study, the minimum document frequency is set to $10$ (\code{min\_df=10}) to discard extremely rare $n$-grams, and a maximum document frequency threshold of $70\%$ (\code{max\_df=0.7}) is established to filter out omnipresent combinations.

\subsubsection{Statistical Pruning} \label{sec:stat_pruning}
\jsremark{Nowe oznaczenia $\feature$ i $\vocab$ - opis na czym jest wzorowana ta implementacja}
A custom implementation of statistical pruning discards neutral noise and retains only the discriminative $n$-grams based on smoothed log-odds $\zscore$-scores, adapted from Monroe et al.~\cite{monroe2008fightin}. The proposed process treats every $n$-gram as a feature $\feature$ which contributes to the feature set, also known as the vocabulary $\vocab$. Each feature $\feature$ is evaluated by comparing its relative frequency between the positive class ($\positive$) and the negative class ($\negative$) through the following steps:

\paragraph{Smoothed Probabilities}

The class $\tokenclass$ acts as a feature generator. The smoothed probability $\smoothedprob{\feature}{\tokenclass}$ of drawing the feature $\feature$ when sampling from class $\tokenclass$ is calculated as follows:
\begin{equation}
	\smoothedprob{\feature}{\tokenclass} = \frac{\Count{\feature}{\tokenclass} + \smoothparam}{\numbertokens{\tokenclass} + \smoothparam \vocabsize},
\end{equation}
where:
\begin{itemize}
	\item $\Count{\feature}{\tokenclass}$ is the number of all occurrences of feature $\feature$ in class $\tokenclass$,
	\item $\smoothparam$ is the additive smoothing parameter. Lidstone smoothing ($\smoothparam=0.01$) is applied to prevent undefined logarithms resulting from zero-frequency occurrences,
	\item $\numbertokens{\tokenclass}$ is the accumulated number of all feature occurrences in class $\tokenclass$ for every feature in the vocabulary $\vocab$, and
	\item $\vocabsize$ stands for the size of the vocabulary $\vocab$.
\end{itemize}

\paragraph{Computing the ${\zscore}$-Score}
The log-odds ratio of the feature's smoothed probability in the positive class $\smoothedprob{\feature}{\positive}$ versus the negative class $\smoothedprob{\feature}{\negative}$ is calculated. The $\zscore$-score normalizes this log-odds ratio by its standard error:
\begin{equation}
	\zscore_\feature = \frac{\ln\frac{\smoothedprob{\feature}{\positive}}{\smoothedprob{\feature}{\negative}}}{\sqrt{\frac{1}{\Count{\feature}{\positive} + \smoothparam} + \frac{1}{\Count{\feature}{\negative} + \smoothparam}}}.
	\label{eq:zscore}
\end{equation}

\paragraph{Thresholding}
Finally, features whose absolute $\zscore$-score falls below a defined threshold are discarded. The cut-off value can be treated as a hyperparameter and is optimized during the experimental phase. This pruning approach preserves only the sub-vocabulary that demonstrates statistically significant separation in feature distribution between the two classes (e.g., a threshold of $|\zscore| \le 1.96$ corresponds to a $95\%$ confidence level). \jsremark{dodałem przykład}
...
\begin{itemize}
	...
	\item {Vectorization type:} BoW and TF-IDF (custom implementations).
...
\begin{figure*}[tbp]
	...
	\begin{tikzpicture}[node distance=2em and 9em]
		...
		node[midway, right, font=\scriptsize] {if true (uncertain)};
		
		% delegation to output
		\draw[arrow] (delegation.east) -| (pred.north) 
		node[pos=0.25, above, font=\scriptsize] {if false (certain)};
...
\jsremark{zmiana opisu: dodałem q oraz Q, usunąłem $\chi^2_{obs}$}
...
The implementation follows \texttt{statsmodels}, using the $\chi^2$ approximation with a continuity correction (\code{exact=False} and \code{correction=True}).
Raw $\mcnemarp$-values are reported without multiplicity correction, given the sequential, hypothesis‑driven design.

To construct the statistical test, let $\missclassified{10}$ be the number of instances misclassified by the baseline only, and $\missclassified{01}$ the number misclassified by the challenger only.
The null hypothesis is $\nullhypo: \missclassified{10} = \missclassified{01}$ (the two classifiers have the same error distribution). 

Let $\mcnemarstat$ be the computed value of the test statistic:
\begin{equation}
	\mcnemarstat = \frac{(|\missclassified{10} - \missclassified{01}| - 1)^2}{\missclassified{10} + \missclassified{01}}.
\end{equation}

Under the null hypothesis $\nullhypo$, the test statistic (denoted as the random variable $\mcnemarstatrand$) approximately follows a chi-squared distribution with one degree of freedom ($\mcnemarstatrand \sim \mcnemarchi_1$).

The $\mcnemarp$-value is the probability that a random variable $\mcnemarstatrand$ would be at least as large as the observed value $\mcnemarstat$.
In the code, this is computed via the survival function: \code{stats.chi2.sf(}$\mcnemarstat$\code{, df=1)}.
If $\mcnemarp < 0.05$, the difference between the two classifiers is considered statistically significant.
Because the statistic squares the absolute difference, the test is two‑sided: it detects any deviation from $\nullhypo$, irrespective of an increase or decrease in accuracy.
...
\begin{figure*}[tbp]%[htbp]
	...
	\caption{Comparison of macro $F_1$-scores across different cascade system configurations. The horizontal dashed line denotes the SVM baseline performance (0.9131). All results show statistical significance relative to the baseline ($\mcnemarp < 0.05$). The exact scores are annotated above each bar. A star marker highlights the top-performing system configuration.}
...
Statistical pruning based on $\zscore$-scores works effectively: the ``not sentiment’’ granule is discarded, leading to a more compact and equally or more accurate feature representation.

\subparagraph{RQ2:} \emph{How can a three-way decision cascade architecture combine the efficiency of lightweight classifiers with the semantic understanding of Transformer-based models?}

A cascade ensemble can be built to improve the overall performance, with the base SVM handling the majority of instances and the specialist transformer resolving only the hardest cases.
