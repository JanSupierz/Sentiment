...
\shorttitle{Granular Computations for Sentiment Detection in Natural Language Texts}
\shortauthors{J.K. Krishnan et~al.}

\title [mode = title]{Granular Computations for Sentiment Detection in Natural Language Texts}                      
\tnotemark[1,2]
...
\ksremark{Przerobię wstęp.}
\jsremark{To jest makro dla Pana :-)}

\ksremark{Może trochę zmodyfikujemy tytuł.}

\subsection{Sentiment Analysis} %introduction into the problem domain
...
\subsection{Problem Statement} %settling of the problem in the domain
...
\subsection{Author's Contribution} %description of contribution of the thesis's author

The contributions of this thesis are twofold: 

\subparagraph{First,} an evaluation of Granular Computing (GrC) methods for SA is performed. Statistical $\zscore$-score pruning of $n$-gram features is shown to reduce the feature space while preserving or even improving classification accuracy. In contrast, semantic clustering into concept granules produces interpretable, but accuracy-limited representations; even when augmented with sentiment lexicon scores. 

\subparagraph{Second,} a cascade ensemble architecture is proposed that uses three-way decisions paradigm at the model level. A lightweight base model handles confident predictions, and a specialist resolves uncertain cases. On the IMDb dataset, this cascade achieves a macro $F_1$-score of approximately $92.3\%$, outperforming standalone baselines, while delegating only 15-20\% of instances to the deep-learning model.

\section{State of the Art} \label{chap:state-of-the-art}
...
\subsubsection{Sentiment Analysis Levels}
...
\subsubsection{Sentiment Analysis Tasks}
...
\subsubsection{Sentiment Analysis Challenges}
...
\paragraph{Dictionary-Based versus Corpus-Based Approaches}
...
\paragraph{Semantic Orientation and Pointwise Mutual Information}
...
\paragraph{Handling Sentiment Shifters and Composition}
...
\paragraph{Preprocessing Considerations}
...
\subsubsection{Classic Machine Learning Models}
...
\paragraph{Coarse-Grained versus Fine-Grained Supervised Learning}
...
\subsubsection{Deep Learning for Sentiment Analysis}
...
\paragraph{Long Short-Term Memory and Gated Recurrent Unit}
...
\section{Granular Cascade System for Sentiment Analysis using Enumerations} \label{chap:system}
...
\paragraph{Computing the $\mathbf{\zscore}$-Score}
...
\subparagraph{Support Vector Machine.} This model minimizes the squared hinge loss with $L_2$ regularization. The default penalty $C=1.0$ is retained, and the solver’s iteration limit is set to $10{,}000$ to ensure convergence on high-dimensional data. Because the final cascade architecture requires calibrated probabilities, a Platt scaling calibrator (\texttt{CalibratedClassifierCV}) is wrapped around the SVM. This post‑processing step maps the decision values to $[0,1]$, producing approximate posterior probabilities for the positive class. Two kernel types are employed: a linear kernel and the Radial Basis Function (RBF) kernel, also known as the Gaussian kernel.

\subparagraph{Logistic Regression Model.} directly models $\probLarge{}{\inputtext}$ by minimizing the log-loss (cross-entropy) with an $L_2$ penalty. It is configured with the \code{L-BFGS} solver, $C=1.0$, and a maximum of $1{,}000$ iterations. Unlike the SVM, \logreg{} natively outputs probability estimates without an external calibration step.
...
\subparagraph{Accuracy} is the fraction of correct predictions:
\begin{equation}
	\text{Accuracy} = \frac{1}{\numVal} \sum_{\idx=1}^{\numVal} \mathbf{1}[\hat{\predVal}_{\idx} = \predVal_{\idx}],
\end{equation}

\subparagraph{Macro $\mathbf{F_1}$-score} averages the $F_1$ score over the two classes without weighting.
