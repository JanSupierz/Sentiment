...
\begin{abstract}
	...
	The main contribution of this work is an optimized lightweight cascade model, where a linear support vector machine handles the majority of reviews with high confidence, while a specialist transformer model resolves only the remaining uncertain cases. 
...
\begin{figure*}[htbp]
	...
	\begin{tikzpicture}[node distance=0.6cm and 1.2cm]
		...
		node[midway, above, font=\scriptsize\bfseries] {if uncertain};
...
A custom implementation of statistical pruning retains only highly discriminative $n$-grams based on smoothed log-odds $\zscore$-scores, adapted from Monroe et al.~\cite{monroe2008fightin}. The proposed process treats every $n$-gram as a feature which contributes to the set of features, the vocabulary $v$. Each feature $\feature$ from the vocabulary $\vocab$ is evaluated by comparing its relative frequency between the positive class ($\positive$) and the negative class ($\negative$) through the following steps:

\paragraph{Smoothed Probabilities}

Each class $\tokenclass$ acts as a feature generator. The smoothed probability $\smoothedprob{\feature}{\tokenclass}$ of drawing feature $\feature$ when sampling from class $\tokenclass$ is calculated as follows:
\begin{equation}
	\smoothedprob{\feature}{\tokenclass} = \frac{\Count{\feature}{\tokenclass} + \smoothparam}{\numbertokens{\tokenclass} + \smoothparam \vocabsize},
\end{equation}
where:
\begin{itemize}
	\item $\Count{\feature}{\tokenclass}$ is the number of occurrences of feature $\feature$ in class $\tokenclass$,
	...
	\item $\numbertokens{\tokenclass}$ is the accumulated number of feature occurrences in class $\tokenclass$ across the entire vocabulary $\vocab$, and
...
Finally, features whose absolute $\zscore$-score falls below a defined threshold are discarded. The cut-off value can be treated as a hyperparameter, and is optimized during the experimental phase. This approach preserves only the vocabulary that demonstrates statistically significant separation in feature distribution between the two classes (e.g., $|\zscore| \le 1.96$ corresponds to a 95\% confidence level).
...
The $\mcnemarp$-value is the probability that a $\mcnemarchi$ random variable would be at least as large as the observed value $\mcnemarchi_{\text{obs}}$.
