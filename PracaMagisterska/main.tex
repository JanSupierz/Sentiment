...
The experimental workflow is designed to isolate and evaluate the impact of each pipeline component. Because the datasets are balanced through stratified sampling, models can be evaluated using both Accuracy and the Macro F1-score. The resulting scores remain very similar.

To ensure that observed performance gains are not attributable to random variance, statistical significance testing is applied to the classifier predictions using the McNemar test by comparing model pairs---baseline vs.\ challenger ($\alpha = 0.05$). The baseline is established dynamically; for any given ablation study or comparison, a new baseline is chosen, which is often the best-performing model from the previous step.
