...
\paragraph{First,} an evaluation of GrC \ksremark{Każdy skrót przy pierwszym wystąpieniu trzeba rozwinąć.} methods for SA is performed. Statistical $\zscore$-score pruning of $n$-gram features is shown to reduce the feature space while preserving or even improving classification accuracy. In contrast, semantic clustering into concept granules produces interpretable, but accuracy-limited representations; even when augmented with sentiment lexicon scores. 
...
Because the data splits are exactly balanced through stratified sampling, accuracy and macro $F_1$-score \ksremark{Trzeba podać wzory na accuracy i macro $F_1$-score.} are equally reliable indicators of performance. Both are reported in the experiments. The primary metric for selecting the best configuration is accuracy, as it directly aligns with the McNemar test.
...
\begin{figure}[tbp]%[htbp]
	...
	\caption{Performance and dimensionality trends across varying $n$-gram ranges. The left panel illustrates model performance (macro $F_1$-score), where dashed lines indicate the respective baselines for Linear SVM (blue) and \logreg{} (red). Hollow markers denote results that are not statistically significantly different from their baseline ($\mcnemaralpha \ge 0.05$), and a star highlights the overall best-performing configuration. The right panel highlights the feature space reduction, comparing the base vocabulary size (green bars) against the pruned vocabulary using $\zscore=2.0$ (orange bars).}
	\label{fig:vocabulary_depth}
\end{figure}

\subsection{Impact of Kernel} \label{sec:Impact-of-Kernel}

To assess the influence of the kernel function, a linear SVM is compared against an RBF SVM, using the best previously determined text representation and preprocessing settings (see Table~\ref{tab:McNemarSvms}). Both models are trained with $5$-fold cross-validation and a fixed \code{max\_iter} limit. The linear SVM with \code{max\_iter=10000} converges in only $1.89$ seconds, achieving $91.34\%$ accuracy. \ksremark{Mamy 5-krotną walidację krzyżową. I mamy średnią dokładność. A jakie jest odchylenie standardowe?}
...
\begin{figure}[tbp]%[htbp]
	...
	\caption{Comparison of macro $F_1$-scores across different cascade system configurations. The horizontal dashed line denotes the SVM baseline performance (0.9131). Bar colors indicate statistical significance relative to the baseline (blue for $\mcnemaralpha < 0.05$, grey for insignificant results), and exact scores are annotated above each bar. A star marker highlights the top-performing system configuration.}
