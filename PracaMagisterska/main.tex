...
While the original IMDb dataset is perfectly balanced and pre-divided into equal training and testing sets, this study merges the sets to perform data deduplication based on exact byte‑level matches of the raw review text. This process identified and removed 418 duplicate entries, resulting in a refined, unique dataset of 49,582 reviews. The dataset is shuffled and re‑divided into a training set of 24,791 reviews (approx. 50\%), a validation set of 17,353 reviews (approx. 35\%), and a testing set of 7,438 reviews (approx. 15\%). This split uses label stratification to maintain a balanced distribution of positive and negative sentiments across all sets.

The textual data is then processed in parallel using a process pool executor to maximize efficiency. Each review is processed sequentially as follows:

\begin{enumerate}
	\item \textbf{Unicode normalization}: the text is converted to Normalization Form Compatibility Composition (NFKC) form to ensure consistent character representation.
	
	\item \textbf{Typographical unification}: a translation table replaces various quote marks (e.g., curly quotes, backticks, acute accents), dashes (en dash, em dash, minus signs), and ellipsis characters with default ASCII equivalents (straight quotes, hyphens, and ``...'' respectively).
	
	\item \textbf{Tag removal}: any substring within the HTML tags $<>$ is replaced with a space.
	
	\item \textbf{Whitespace condensation}: multiple whitespace characters are collapsed into a single space, and leading/trailing spaces are stripped.
	
	\item \textbf{Contraction expansion}: English contractions are expanded using the \texttt{contractions.fix} function (e.g., ``don't'' $\rightarrow$ ``do not'').
	
	\item \textbf{Tokenization}: the expanded text is tokenized using NLTK's \texttt{word\_tokenize}, producing \texttt{tokens\_cased}.
	
	\item \textbf{Lowercasing}: each token in \texttt{tokens\_cased} is converted to lowercase, producing \texttt{tokens\_lower}.
	
	\item \textbf{Part‑of-Speech-filtered tokenization}: the expanded text is processed with spaCy's English model (parser and named entity recogniser disabled). Tokens are lowercased and filtered based on their part‑of‑speech (POS) tags, as shown in the Table~\ref{tab:pos-filtering}. The result is stored as \texttt{tokens\_filtered}.
	
	\item \textbf{Alphabetic filtering}: from \texttt{tokens\_lower}, any character that is not a letter (a–z, A–Z) is removed; tokens that become empty after this removal are discarded. The result is \texttt{tokens\_letters}.
	
	\item \textbf{Stemming}: the Porter stemmer is applied to \texttt{tokens\_lower}, producing \texttt{tokens\_stemmed}.
	
	\item \textbf{Lemmatization}: the WordNet lemmatizer is applied to \texttt{tokens\_lower}, producing \texttt{tokens\_lemmatized}.
\end{enumerate}

After processing, each review is stored as a dictionary containing the different representations. The processed datasets are then converted to Pandas DataFrames, selecting a consistent column order, and finally saved as Parquet files (\texttt{train.parquet}, \texttt{val.parquet}, \texttt{test.parquet}) to enable selective loading in subsequent experiments.

\subsubsection{Filtering Parts-Of-Speech}

The tokens are lowercased and filtered based on their POS description. This step can highly lower the $n$-gram dimensionality in the upcoming steps by removing tags which do not influence the sentiment. However, this preprocessing step is purely subjective guesswork. In this study, it was decided to keep the filtering to the very minimum, as sentiment can be very subtle and hidden in the wording.
...
The POS filtering step removes tokens according to three criteria:

\begin{itemize}
	\item \textbf{Auxiliary verbs (except modals)}: tokens with the coarse POS tag \texttt{AUX}, unless their fine-grained tag is \texttt{MD} (modal auxiliary, e.g., \textit{can}, \textit{will}, \textit{must}).
	\item \textbf{Determiners}: tokens with the coarse tag \texttt{DET} (e.g., \textit{a}, \textit{an}, \textit{the}, \textit{many}, \textit{my}).
	\item \textbf{Possessive endings}: tokens with the fine-grained tag \texttt{POS} (the possessive clitic \textit{'s}).
\end{itemize}

\subsection{Feature engineering}

To evaluate the impact of vocabulary representation on model performance, the tokenized text is transformed into discrete feature spaces using combinations of unigrams, bigrams, and trigrams. However, higher-order $n$-grams naturally introduce massive, highly sparse feature spaces. To mitigate the curse of dimensionality, an attempt is made to make use of a few advanced dimensionality reduction techniques: custom vectorization, statistical pruning and semantic clustering.

\subsubsection{Custom Vectorization}

The default \texttt{CountVectorizer} from scikit-learn applies pruning thresholds like \texttt{min\_df} and \texttt{max\_df} to individual words before building $n$-grams. This can discard rare unigrams that might form informative higher-order $n$-grams. To overcome this limitation, a custom pipeline is implemented.

The input to this stage is already tokenized: each review is represented as a list of string tokens. These lists are joined back into single strings with spaces between tokens. A specialized tokenizer, \texttt{space\_tokenizer}, is then provided; it bypasses the default regular-expression tokenizer and splits only on spaces. The vectorizer is configured with \texttt{lowercase=False} (since lowercasing has already been applied during preprocessing) and the desired \texttt{ngram\_range} (e.g., (1,3)).

This design ensures that the \texttt{min\_df} and \texttt{max\_df} thresholds are applied to complete $n$-grams, not to individual words before $n$-gram formation. Consequently, important multi-word expressions are preserved. For this study, $n$-grams that appear in fewer than 10 documents are discarded (\texttt{min\_df=10}), and no upper frequency limit is set (\texttt{max\_df=1.0}).

\subsubsection{Statistical Pruning}
A custom statistical pruning method based on Z-scores is employed to filter out neutral noise and retain only highly discriminative vocabulary. For a given feature, the log-odds ratio of its occurrence in the positive class versus the negative class is calculated. To prevent undefined logarithms resulting from zero-frequency occurrences, Laplace smoothing ($\alpha = 0.01$) is applied. The Z-score for a feature is calculated as:
...
\begin{enumerate}
	...
	\item \textbf{Uncertainty delegation:} A delegation threshold $t_{\text{lower}}$ is defined. If the base model's probability falls within the uncertain interval $t_{\text{lower}} \le P(y=1 \mid X)_{\text{base}} \le 1 - t_{\text{lower}}$, the prediction is considered unreliable and is deferred. The threshold is selected by optimising the trade‑off between coverage and accuracy on the validation set.
	
	\item \textbf{Specialist resolution:} Uncertain instances are delegated to a computationally heavy specialist model. An attempt is made to further fine‑tuned the specialist on challenging samples mined from the validation set - specifically, the subset of data that the base model failed to classify confidently.
\end{enumerate}
...
The experimental workflow is designed to systematically isolate and evaluate the impact of each pipeline component. Models are evaluated using the Macro F1-score, ensuring a balanced assessment across both positive and negative classes regardless of slight distribution shifts. 

To ensure that observed performance gains are not attributable to random variance, statistical significance testing is applied to the classifier predictions using the McNemar test ($\alpha = 0.05$) between pairs samples. The baseline is established dynamically - for any given ablation study or comparison, the best-performing preceding configuration is selected as the baseline.

\subsection{Implementation}

All experiments, data transformations, and statistical analyses are implemented using Python. To ensure reproducibility and computational efficiency, the architecture leverages open-source libraries across various domains of the experimental pipeline:
...
As illustrated in Figure~\ref{fig:casing-expansion-effect}, the transition from cleaned to expanded text provides a statistically significant benefit for lower-cased text, as shown by means of the McNemar's test, applied to the pairwise predictions.

\subsubsection{Comparative Analysis of Configurations}

Tables~\ref{tab:McNemarE1a} through \ref{tab:McNemarE1d} provide a cross-configuration analysis, comparing various models and representations against selected baselines. From these results, several key trends emerge:

\begin{enumerate}
	\item \textbf{Model Impact:} Linear SVM consistently outperforms LogReg models across nearly all equivalent settings. For instance, as shown in Table~\ref{tab:McNemarE1a}, the SVM baseline ($90.61\%$) is significantly superior to its LogReg counterpart ($89.75\%$, $p < 0.001$).
	\item \textbf{Representation Impact:} TF-IDF features yield better performance than BoW. This delta is statistically significant in both cased and lowercased scenarios (see Tables~\ref{tab:McNemarE1a} and \ref{tab:McNemarE1b}).
	\item \textbf{Synergy of Lowering and Expansion:} The most notable performance gain occurs when expansion is applied to lowercased text. Table~\ref{tab:McNemarE1b} highlights that the "Expanded and Lower" configuration achieves the highest accuracy ($90.98\%$), representing a significant improvement over the standard "Lower" baseline ($p = 0.0369$) - as already depicted in the Figure~\ref{fig:casing-expansion-effect}.
\end{enumerate}
...
\subsection{Impact of Statistical Pruning}
