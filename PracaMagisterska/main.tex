...
\newcommand{\probSmall}[1]{p_{#1}}       % e.g., p_B, p_S
\newcommand{\probLarge}[2]{P_{#1}(\predVal=1 \mid #2)}

% Sweep and metrics
\newcommand{\numVal}{N}
\newcommand{\sweepthresh}{t}
\newcommand{\acceptance}[1]{A_{\idx}(#1)}   % acceptance mask for instance i with threshold #1
\newcommand{\coverage}{\epsilon(\sweepthresh)}
\newcommand{\hybridacc}{\aleph(\sweepthresh)}
\newcommand{\cascadePred}[1]{\hat{\predVal}_{\idx}^{\,\mCas}(#1)} % cascade prediction for i with threshold #1
...
\chapter{Cascade System for Granular Sentiment Analysis using Grid-Search} \label{chap:system}
...
\subsection{Grid-Search Strategy} \label{sec:grid_strategy}

To navigate the large design space created by the preceding options---token representations, $n$-gram ranges, pruning thresholds, clustering granularities, and sentiment weights---a structured grid search is employed. The search evaluates every combination of the following axes:
...
\end{itemize}
Not all combinations are independent: when $\kconcepts=0$ the sentiment weight is irrelevant. The grid is executed in stages: first, a broad sweep identifies the most promising token representations and pruning thresholds; second, a focused search on the surviving configurations tests vocabulary depth and concept-based compression. The overall best configuration---a linear SVM with stemmed tokens, TF-IDF, and a \zscore-score threshold of $\zscore=2$—is then used as the base model in the cascade ensemble. DistilBERT is evaluated outside the grid, serving as a standalone deep-learning benchmark and, later, as a cascade specialist.
...
The delegation threshold $\thresh$ governs the trade-off between the fraction of instances processed by the fast base model (coverage) and the overall performance of the hybrid system (global accuracy). To determine the optimal value, a sweep over $\thresh$ is performed on the validation set. For each candidate threshold, the cascade predictions are simulated using the best base model achieved from the grid search and the generic DistilBERT variant. The generic model serves as a heuristic to establish reasonable coverage boundaries.

Let the validation set contain $\numVal$ instances. For instance $\idx$, denote the base model's output by $\probSmall{\mBase}^{(\idx)} = \probLarge{\mBase}{\inputtext_{\idx}}$, which corresponds to evaluating $\probSmall{\mBase}$ on that specific input. For a candidate threshold $\sweepthresh \in [0,0.5]$, the acceptance mask for instance $\idx$ is
\begin{equation}
	\acceptance{\sweepthresh} = \mathbf{1}\bigl[\probSmall{\mBase}^{(\idx)} \le \sweepthresh \;\lor\; \probSmall{\mBase}^{(\idx)} \ge 1-\sweepthresh \,\bigr].
\end{equation}
The coverage is defined as
\begin{equation}
	\coverage = \frac{1}{\numVal} \sum_{\idx=1}^{\numVal} \acceptance{\sweepthresh},
\end{equation}
and the hybrid accuracy is
\begin{equation}
	\hybridacc = \frac{1}{\numVal} \sum_{\idx=1}^{\numVal} \mathbf{1}\bigl[\, \cascadePred{\sweepthresh} = \predVal_\idx \,\bigr],
\end{equation}
where $\cascadePred{\sweepthresh}$ is the final cascade prediction for instance $\idx$ when using threshold $\sweepthresh$.  

The optimal threshold is chosen as
\begin{equation}
	\thresh = \arg\max_{\sweepthresh \in [0,0.5]} \hybridacc,
\end{equation}

The sweep yields $\thresh = 0.254$, which defines an acceptance region of $[0,0.254) \cup (0.746,1]$ (width $\approx 0.492$). At this operating point, roughly 25~–~30\% of the test instances are expected to be delegated to the specialist.
...
\subsection{Experimental Sequence of the Grid Search}

The grid-search space defined in Section~\ref{sec:grid_strategy} is explored through a sequence of experiments. Each experiment produces a ranking of configurations from which the single best-performing model is selected.

\begin{enumerate}
	\item \textbf{Preprocessing and basic representation.} Using the default \texttt{CountVectorizer} with $(1,3)$ $n$-grams, the four combinations of text variant (clean vs.\ expanded) and casing (cased vs.\ lowercased) are evaluated with both BoW and TF-IDF for the linear SVM and \logreg{}. This stage identifies the best preprocessing choice (expanded, lowercased, TF-IDF) and confirms the superiority of the linear SVM.
	...
	\item \textbf{Cascade threshold sweep.} The best SVM configuration from the grid search is used as the base model. A sweep over $\thresh \in [0,0.5]$ is performed on the validation set, simulating the cascade with the generic DistilBERT variant (Section~\ref{sec:threshold_selection}). The threshold that maximizes hybrid accuracy, $\thresh = 0.254$, is selected.
\end{enumerate}
...
To verify that the delegation threshold $\thresh$ genuinely isolates difficult cases, the base model's predicted probabilities are binned and the empirical accuracy in each bin is examined. For instances with probabilities close to $0$ or $1$ (the acceptance regions), the observed accuracy is substantially higher than for instances with probabilities near $0.5$ (the boundary region). This confirms that the SVM's Platt-scaled outputs are well-calibrated and that the interval $[\thresh,\,1-\thresh]$ captures cases where the base model is most error-prone.

\subsubsection{Specialist Comparison}

Three cascade variants are evaluated to assess which specialist configuration yields the best performance. The base model is fixed to the optimal linear SVM identified by the grid search (Section~\ref{sec:grid_strategy}), and the delegation threshold is set to $\thresh = 0.254$ (Section~\ref{sec:threshold_selection}). Three specialist variants are tested:
...
\subsection{Visualisation of Statistical Significance}
...
\begin{itemize}
	...
	\item \textbf{Tabular reports.} Rows with $p < 0.05$ are set in bold and accompanied by a directional arrow ($\uparrow$ for improvement, $\downarrow$ for degradation). The baseline row is also bold. Rows with $p \ge 0.05$ use regular weight and no arrow, as illustrated in Table~\ref{tab:McNemarExample}.
\end{itemize}
