...
\begin{enumerate}
	...
	\item \textbf{POS-filtered tokenization} processes the expanded text by filtering \code{tokens\_lower} based on their Part-Of-Speech (POS) tags; the result is stored as \code{tokens\_filtered}.
...
The classification task is approached through a comparative analysis of traditional machine learning algorithms and a transformer-based deep neural network. All models are trained on the preprocessed training set, with hyperparameter selection and early stopping guided by a held-out validation partition.

\subsection{Traditional Baselines}

A linear SVM and \logreg{} are employed as computationally efficient baselines. Both classifiers are trained on the feature matrices produced by the preprocessing and feature engineering stages---specifically, the TF-IDF weighted $n$-gram vectors (BoW representations are also evaluated during experimentation, but the core pipeline converges on TF-IDF). The training procedure utilizes the scikit-learn implementations with the following configuration.

\subsubsection{Linear Support Vector Machine}

The linear SVM optimizes the squared hinge loss with L2 regularization. The default penalty strength $C=1.0$ is retained, and the solver is permitted up to $10\,000$ iterations to ensure convergence even on high-dimensional data. At inference time, the raw signed distance to the separating hyperplane is not a calibrated probability measure. To embed the SVM into the cascade architecture, which requires reliable confidence estimates, the distance scores are passed through a Platt scaling calibrator (\texttt{CalibratedClassifierCV}). This wrapper fits an internal \logreg{} on the SVM scores using $5$-fold cross-validation, thereby mapping unbounded decision values to the $[0,1]$ interval and producing well-calibrated posterior probabilities for the positive class.

\subsubsection{Logistic Regression}

\logreg{} directly models the conditional class probability $P(y=1 \mid \inputtext)$ by minimising the log-loss (cross-entropy) objective with an L2 penalty. It is configured with the L-BFGS solver, which approximates the Hessian for quasi-Newton optimisation and is well-suited to moderately sized problems. The regularization strength $C=1.0$ and a maximum of $1\,000$ iterations are used. Unlike the SVM, LR natively outputs probability estimates without an external calibration step. Both models are fitted on the same training split and evaluated on the same validation and test sets, ensuring fair comparison.
	
\subsection{Deep Learning Classifier}

The deep learning component of the system is based on DistilBERT, a distilled, six-layer variant of the BERT architecture that preserves over 95\% of the original model’s performance while being substantially smaller and faster. Two separate training regimes are employed: a \textbf{basic fine-tuning} that yields a standalone sentiment classifier for benchmark comparisons, and a \textbf{specialist fine-tuning} that adapts the model to the ambiguous instances encountered in the cascade ensemble.

\subsubsection{Basic Fine-Tuning}
\label{sec:bert_basic}

The checkpoint \texttt{distilbert-base-uncased-finetuned-sst-2-english}, which has been pre-trained on large English corpora and already fine-tuned on the Stanford Sentiment Treebank (SST-2), is loaded from the Hugging Face hub. Its classification head is further fine-tuned on the full IMDb training set. Input texts (the \code{text\_clean} field) are tokenized with the corresponding DistilBERT tokenizer, truncated or padded to a maximum length of $256$ tokens. Training is performed for $3$ epochs with a batch size of $16$, using the Adam optimizer with a learning rate of $2 \times 10^{-5}$. Mixed-precision (float16) computation is utilized to reduce memory consumption and accelerate training. Early stopping monitors the validation loss with a patience of $2$ epochs, and a learning rate reduction on plateau (factor $0.5$, patience $1$) is applied. A stratified $20\%$ hold-out from the training set serves as the validation partition. After training, the model outputs class probabilities via a softmax over the two logits. This basic DistilBERT serves two purposes: it provides a strong baseline for the standalone comparison of traditional models versus deep learning, and it is reused as the initialisation point for the specialist variant.

\subsubsection{Specialist Fine-Tuning}
\label{sec:bert_specialist}

For the cascade ensemble, the basic DistilBERT model is further fine-tuned on the subset of validation instances that the base model could not classify with high confidence. The backbone of the transformer is partially frozen: the first $5$ layers are kept frozen, while the remaining layers and the classification head remain trainable. The specialist is trained for $10$ epochs with a lower learning rate of $2 \times 10^{-6}$, a batch size of $16$, and early stopping with a patience of $3$ epochs. All other training settings (mixed precision, tokenizer, maximum sequence length) remain identical to the basic fine-tuning. This targeted fine-tuning encourages the model to specialise in the ambiguous region of the decision boundary identified by the base model, without overfitting to the relatively small hard-sample set.

\section{Cascade Ensemble}

To balance the computational efficiency of traditional machine learning algorithms with the deep understanding capacity of transformer-based models, a cascade ensemble architecture is proposed. The design follows the 3WD paradigm, where each instance is assigned to one of three regions based on the predictive confidence of a lightweight base model. Instances classified with high confidence are accepted immediately, while those falling into a boundary region of uncertainty are deferred to a more powerful but computationally expensive specialist.
...
\begin{enumerate}
	\item \textbf{Initial inference:} A computationally inexpensive base model---selected through the feature engineering and model selection pipeline---evaluates an input text $\inputtext$ and outputs a probability score $P(y=1 \mid \inputtext)_{\text{base}} \in [0,1]$.
	
	\item \textbf{Uncertainty delegation:} A threshold $t_{\text{lower}} \in [0, 0.5]$ is defined. If the base model's probability lies within the closed interval
	\begin{equation}
		t_{\text{lower}} \le P(y=1 \mid \inputtext)_{\text{base}} \le 1 - t_{\text{lower}}
	\end{equation}
	the prediction is considered unreliable and the instance is placed in the boundary region. The complementary intervals $[0, t_{\text{lower}})$ and $(1-t_{\text{lower}}, 1]$ correspond to high-confidence negative and positive predictions, respectively, which are accepted without further processing.
	...
	\begin{itemize}
		\item an SVM specialist trained exclusively on the boundary samples mined from the validation set;
		\item a generic DistilBERT classifier (basic fine-tuning, Section~\ref{sec:bert_basic}) used as a black-box specialist;
		\item a DistilBERT specialist fine-tuned on the same boundary samples with the backbone partially frozen (specialist fine-tuning, Section~\ref{sec:bert_specialist}).
	\end{itemize}
\end{enumerate}

The final prediction for each \inputtext{} is obtained by combining the outputs:
\begin{equation}
P(y=1 \mid \inputtext)_{\text{cascade}} =
\begin{cases}
	P(y=1 \mid \inputtext)_{\text{base}}, & \text{if \inputtext{} is in the acceptance region},\\[4pt]
	P(y=1 \mid \inputtext)_{\text{specialist}}, & \text{if \inputtext{} is in the boundary region}.
\end{cases}
\end{equation}
The hard class label is derived by thresholding this probability at $0.5$.

\subsection{Threshold Selection}

The delegation threshold $t_{\text{lower}}$ is a critical hyperparameter that governs the trade-off between coverage (the fraction of instances processed by the fast base model) and global accuracy (the overall performance of the hybrid system). To determine the optimal value, a sweep over possible thresholds is performed on the validation set. For each candidate $t_{\text{lower}}$, the cascade predictions are simulated using the base model and the specialist (the basic BERT variant is used as a placeholder during this sweep to avoid data leakage from the final specialist training). Two quantities are recorded: the coverage, i.e., the proportion of instances that are accepted by the base model, and the hybrid accuracy, which is the accuracy of the combined predictions on the entire validation set. The threshold that maximizes hybrid accuracy is selected, provided it does not delegate an excessive fraction of the data (practical coverage constraints are implicitly encoded in the sweep range). The optimal threshold found through this procedure is $t_{\text{lower}} = 0.254$, resulting in an acceptance region width of approximately $0.492$ ($0.254$ to $0.746$). At this operating point, roughly 25--30\% of the test instances are expected to be delegated to the specialist.

\subsection{Specialist Training}

The specialist models are trained on the boundary region samples identified by the base model on the validation set.

For the SVM specialist, the same linear SVM architecture and hyperparameters as the base model are employed, but the model is fitted exclusively on the TF-IDF representations of the validation documents whose base model prediction probability fell within $[t_{\text{lower}}, 1-t_{\text{lower}}]$. This creates a lightweight specialist that shares the same feature space and decision function type as the base model but is concentrated on the ambiguous region.

For the BERT specialist, the fine-tuning procedure detailed in Section~\ref{sec:bert_specialist} is applied to the same boundary subset. No additional data balancing is required, as the boundary region naturally contains an approximately equal number of positive and negative examples due to the symmetric thresholding.

\subsection{Evaluation}

The final cascade predictions are evaluated on the held-out test set. Performance is compared against the standalone base model, the standalone BERT baseline, and an ensemble that uses the basic BERT without specialist fine-tuning. McNemar's test is applied to the paired predictions to assess statistical significance. This architecture ensures that the majority of clear-cut reviews are processed rapidly by the linear SVM, while the GPU-intensive transformer inference is reserved exclusively for the small fraction of texts for which the base model's confidence is insufficient.
...
\begin{itemize}
	...
	\item \textbf{Statistical Testing and Visualization:} McNemar's test is computed using \code{statsmodels}. All graphical representations are generated with \code{matplotlib}, \code{seaborn}, and the \code{wordcloud} library.
\end{itemize}
...
The experimental workflow is designed to isolate the impact of each pipeline component. To ensure that observed performance gains are statistically significant rather than artifacts of random variance, McNemar's test ($\mcnemaralpha = 0.05$) is applied to paired classifier predictions (baseline versus challenger). For any ablation study or comparative analysis, the baselines are established dynamically; typically, the best-performing model from the preceding step is chosen.

Unlike tests that compare aggregate scores, McNemar's test analyzes discordant pairs---instances where one model predicts correctly while the other errs. Because the test evaluates the marginal frequencies of correct and incorrect prediction counts, it inherently measures differences in overall classification accuracy. As the datasets in this study are kept strictly balanced through stratified sampling, model performance can be reliably assessed using both accuracy and the macro F1-score. The balance of the data makes accuracy a robust metric, thereby ensuring that McNemar's test is a perfectly aligned statistical approach for this study.
...
Instead of relying on the default \code{CountVectorizer}, a custom implementation is utilized, bypassing the standard regular expression tokenization. Following the vectorization, neutral tokens are pruned based on their calculated \zscore-scores. The new approaches are Custom BoW (\custombow) and Custom TF-IDF (\customtfidf).
...
The statistical analysis reveals an important trend regarding feature pruning. While a few configurations at $\zscore=1$ and $\zscore=2$ yield statistically significant improvements over the baseline ($\uparrow$), the vast majority of combinations between $\zscore=0$ and $\zscore=3$ result in $p$-values greater than $0.05$. This indicates that for most representation strategies, moderate statistical pruning perfectly maintains classification performance on par with the unpruned baseline. Statistically significant degradation ($\downarrow$) only begins to manifest under aggressive pruning at $\zscore=4$.
\begin{table}[htbp]
	\centering
...
This section demonstrates that the \customtfidf{} representation achieves significantly higher accuracy than the \texttt{\custombow{}} approach across all evaluated configurations. This outcome verifies that the pattern observed in the initial experiments---depicted in Tables~\ref{tab:McNemarE1a} through~\ref{tab:McNemarE1d}---holds true: TF-IDF fundamentally outperforms simple BoW for this classification task. Consequently, it can be concluded that the superiority of TF-IDF is robust and largely independent of the tokenization strategy employed. Consistent with the trends previously illustrated in Figure~\ref{fig:zscore_threshold}, aggressive pruning at $\zscore=4$ results in severe performance degradation across all representations.
...
In the ablation study depicted in Figure~\ref{fig:ablation_study}, the BoW and TF-IDF labels denote standard, unpruned token representations. On the other hand, \custombow{} and \customtfidf{} designate configurations where statistical pruning ($\zscore=2$) has been applied.

For the standard BoW representations, the application of various tokenization strategies yields no statistically significant improvement over the \texttt{Expanded and Lower} baseline for either the linear SVM or \logreg{} models. Furthermore, when statistical pruning is introduced (\custombow{}), performance consistently degrades across all configurations; removing vocabulary features universally harms the predictive power of the raw BoW.

In contrast, the TF-IDF representations exhibit a lot of variance. For the linear SVM, only the lemmatized tokenization approach proves significantly better than the TF-IDF Baseline. However, the introduction of $\zscore=2$ pruning actively enhances the model's capabilities, significantly increasing the Macro F1-scores for the lowered and stemmed representations. The combination of stemming and \customtfidf{} achieves the highest overall performance metric ($91.41\%$).

For the \logreg{} model, the overall classification scores are generally lower than those achieved by the SVM. For the BoW approach, pruning either degrades performance (as seen with cased and letter-only tokens) or has no statistically significant effect. However, the introduction of filtering combined with \customtfidf{} pruning proves highly effective. While isolated tokenization approaches do not have a clear impact on their own, the pruning strategy elevates almost all TF-IDF-based representations for \logreg{}, with the sole exception of the \texttt{Tokens Letters} configuration. The \texttt{Tokens Filtered} representation with $\zscore=2$ pruning outperforms the baseline by over 0.7 percentage points (rising from $89.98\%$ to $90.71\%$).

\begin{figure}[tbp]
	\centering
	\includegraphics[width=1\textwidth]{./figures/analysis/ablation_study.png}
	\caption{Ablation study of different token representations evaluating pre-tokenized representations without pruning ($\zscore=0$), and custom representations with statistical pruning ($\zscore=2$). These are compared against baseline \texttt{CountVectorizer} equivalents established in the initial experiments.}
...
Semantic clustering groups semantically related $n$-grams to capture conceptual representations and evaluate the combined impact of similar phrases. For example, as shown in Figure~\ref{fig:concepts_pos_10000_0}, the cluster containing \literal{great}, \literal{wonderful}, and \literal{amazing} forms the most discriminative positive concept, achieving a \zscore-score of 44.20. An interesting pattern emerges with clusters focused on structural phrases, such as \literal{his} or \literal{is an}, which also rank among the top positive concepts. This reinforces the earlier observation that reviewers tend to use more descriptive language when expressing a positive opinion.

Conversely, Figure~\ref{fig:concepts_neg_10000_0} illustrates that negative clusters contain repeated punctuation. A significant advantage of clustering $n$-grams in this manner is that varying permutations of punctuation (e.g., \literal{?!} and \literal{!??}) are effectively grouped into a single conceptual feature, which theoretically could improve the model's ability to generalize noisy text. Other top negative concepts consist of direct negative descriptors (e.g., \literal{worst}, \literal{horrible}, \literal{awful}). The cluster containing the phrase \literal{bad, but} appears highly ranked for negativity, even though such phrasing often introduces a mitigating clause or mixed sentiment in natural language rather than pure negativity.
