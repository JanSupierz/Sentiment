...
\paragraph{First,} an evaluation of GrC methods for SA is performed. Statistical $\zscore$-score pruning of $n$-gram features is shown to reduce the feature space while preserving or even improving classification accuracy. In contrast, semantic clustering into concept granules produces interpretable, but accuracy-limited representations; even when augmented with sentiment lexicon scores. 

\paragraph{Second,} a cascade ensemble architecture is proposed that uses three-way decisions paradigm at the model level. A lightweight base model handles confident predictions, and a specialist resolves uncertain cases. On the IMDb dataset, this cascade achieves a macro $F1$-score of approximately $92.3\%$, outperforming standalone baselines, while delegating only $15-20\%$ of instances to the deep-learning model.
...
\begin{enumerate}
	...
	\item \textbf{POS-filtered tokenization} replaces the standard stop-word removal (Sec.~\ref{sec:stop-word-removal}); the expanded text (\code{tokens\_lower}) is filtered based on the Part-Of-Speech (POS) tags; the result is stored as \code{tokens\_filtered}.
...
Because the data splits are exactly balanced through stratified sampling, accuracy and macro F1-score \ksremark{Trzeba podać wzory na accuracy i macro $F_1$-score.} are equally reliable indicators of performance. Both are reported in the experiments. The primary metric for selecting the best configuration is accuracy, as it directly aligns with the McNemar test.

\subsubsection{Statistical Significance Testing}

All pairwise model comparisons are validated with McNemar's test at $\mcnemaralpha = 0.05$. The test evaluates whether the error distributions of two classifiers, computed on the same set of instances, differ significantly. In each experiment a baseline is chosen---typically the best-performing configuration from the previous test. The continuity-corrected variant of the test is employed (\code{statsmodels}), and raw $\mcnemaralpha$-values \ksremark{$p$-values?} are reported without multiplicity correction, given the sequential, hypothesis-driven design.
...
\begin{itemize}
	\item \textbf{Graphs.} Significant comparisons ($\mcnemaralpha < 0.05$) are drawn with full colour saturation, thick lines, and solid markers; non-significant ones use low opacity, thinner lines, and hollow markers. The legend carries the tags ``Sig.\ Diff'' and ``Not Sig.'' where appropriate.
	\item \textbf{Tabular reports.} Rows with $\mcnemaralpha < 0.05$ are set in bold and accompanied by a directional arrow ($\uparrow$ for improvement, $\downarrow$ for degradation \ksremark{Czy ten test jest na pewno symetryczny? Tzn. czy sprawdzamy, czy $A > B$ (ale nie $A < B$) czy $A \neq B$?}). The baseline row is also bold. Rows with $\mcnemaralpha \ge 0.05$ use regular weight and no arrow, as illustrated in Table~\ref{tab:McNemarExample}.
\end{itemize}
...
\begin{table}[htbp]
	...
	\begin{tabular}{lllrc}
		\toprule
		Model Description & $Acc_{Model}$ & Direction & $\mcnemaralpha$-value \\
		\midrule
...
In this test, the performance of two classical classifiers---linear SVM and \logreg{}---is evaluated across varying preprocessing and feature representation configurations. All models were trained on the training set. The evaluation metrics (Macro \ksremark{małą literą} F1-score and Accuracy \ksremark{małą literą}) are computed on the validation set. The experimental conditions are categorized as follows:
\begin{itemize}
...
\begin{table}[tbp]%[htbp]
	\centering
	\caption{McNemar test results analyzing the impact of casing, text expansion, representation, and model choice. Across all subtables, bold text indicates a statistically significant difference ($\mcnemaralpha < 0.05$) relative to the baseline, with arrows denoting the direction of the change in accuracy.}
	...
	\begin{subtable}{\textwidth}
		...
		\begin{tabular}{llllrcc}
			\toprule
			& Text & Representation & Model & $Acc_{Model}$ & $\uparrow$/$\downarrow$ & $\mcnemaralpha$-value \\
			\midrule
...
While a few configurations at $\zscore=1$ and $\zscore=2$ yield improvements over the baseline ($\uparrow$), the vast majority of combinations between $\zscore=0$ and $\zscore=3$ result in $\mcnemaralpha$-values greater than $0.05$. This indicates that for most representation strategies, moderate statistical pruning perfectly maintains classification performance on par with the unpruned baseline. Significant degradation ($\downarrow$) only begins to manifest under aggressive pruning at $\zscore=4$.
...
\begin{figure}[tbp]%[htbp]
	...
	\caption{Performance and dimensionality trends across varying $n$-gram ranges. The left panel illustrates model performance (Macro $F1$-Score), where dashed lines indicate the respective baselines for Linear SVM (blue) and \logreg{} (red). Hollow markers denote results that are not statistically significantly different from their baseline ($\mcnemaralpha \ge 0.05$), and a star highlights the overall best-performing configuration. The right panel highlights the feature space reduction, comparing the base vocabulary size (green bars) against the pruned vocabulary using $\zscore=2.0$ (orange bars).}
	\label{fig:vocabulary_depth}
\end{figure}

\subsection{Impact of Kernel} \label{sec:Impact-of-Kernel}

To assess the influence of the kernel function, a linear SVM is compared against an RBF SVM, using the best previously determined text representation and preprocessing settings (see Table~\ref{tab:McNemarSvms}). Both models are trained with $5$-fold cross-validation and a fixed \code{max\_iter} limit. The linear SVM with \code{max\_iter=$10,000$} converges in only $1.89$ seconds, achieving $91.34\%$ accuracy. \ksremark{Mamy 5-krotną walidację krzyżową. I mamy średnią dokładność. A jakie jest odchylenie standardowe?}
In contrast, the RBF kernel with the same iteration limit trains for $2042.05$ seconds (over $34$ minutes) and reaches $91.15\%$ accuracy.
However, the RBF solver did not converge: it exhausted the maximum number of iterations before reaching the stopping criterion.
This indicates that the RBF kernel's optimization landscape is harder to navigate, and requires more time for proper convergence.
...
The difference between this RBF variant and the linear baseline is significant according to McNemar's test ($\mcnemaralpha < 0.000001$), whereas the non-converged RBF with $10,000$ iterations does not exhibit a statistically significant difference from the linear SVM ($\mcnemaralpha = 0.142$).
...
\begin{table}[tbp]
	...
	\begin{tabular}{llllrcc}
		\toprule
		& \code{max\_iter} & Training Time & Model & $Acc_{Model}$ & $\uparrow$/$\downarrow$ & $\mcnemaralpha$-value \\
		\midrule
...
\begin{figure}[tbp]%[htbp]
	...
	\caption{Comparison of Macro $F1$-scores across different cascade system configurations. The horizontal dashed line denotes the SVM baseline performance (0.9131). Bar colors indicate statistical significance relative to the baseline (blue for $\mcnemaralpha < 0.05$, grey for insignificant results), and exact scores are annotated above each bar. A star marker highlights the top-performing system configuration.}
...
\ksremark{Trzeba tutaj napisać, o co w ogóle w pracy chodzi.} 

\section{Summary of Work}

This thesis addressed the problem that GrC approaches had not been systematically verified in the SA pipeline. Through an enumeration of preprocessing, representation, and modeling choices, the most effective configurations were identified. The work demonstrated that a statistical $\zscore$-based pruning technique can discard the ``neutral'' granule---the $n$-grams that do not discriminate between positive and negative classes---thereby reducing the feature space while preserving or even improving accuracy. Building on this, a hybrid cascade ensemble was proposed: a lightweight linear SVM acts as a base model, handling the majority of instances with high confidence, while a deep learning specialist (DistilBERT) resolves the remaining uncertain cases. The cascade achieved a macro $F1$-score of approximately $92.3\%$, outperforming both standalone baselines while delegating only $15\%$–$20\%$ of the data to the expensive model.
