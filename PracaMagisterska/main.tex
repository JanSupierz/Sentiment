...
\begin{itemize}	
	...
	\item \textbf{Sarcasm and Irony Detection} presents particularly difficult challenges, as the literal meaning of an utterance contradicts its intended sentiment \cite{behdenna2018document, sharma2025review}. When someone says something positive but actually means something negative, or vice versa, purely lexical approaches inevitably fail \cite{behdenna2018document}. A significant portion of recent research has emphasized the need for improved models capable of classifying sarcasm and irony \cite{sharma2025review}.
...
Generally text is first converted into a numerical representation, typically a bag-of-words (BoW) model where each document is represented by a vector of word frequencies. Variants include $n$-grams (contiguous sequences of words) and TF-IDF (term frequency–inverse document frequency) weighting, which down-weights common words \cite{behdenna2018document}.
...
\begin{itemize}
\item Pang was the first to apply machine learning to movie review classification, showing that binary feature representation outperforms frequency-based representations \cite{behdenna2018document}. \ksremark{Czy mamy dostęp do pracy Panga?}

\item Pak and Paroubek proposed sub-graph representations from syntactic dependency trees, showing that such structured features avoid information loss associated with pure n-gram models and improve SVM performance on French reviews \cite{behdenna2018document}. \ksremark{tutaj i niżej – analogicznie}
...
\ksremark{A jakie są wady tego podejścia?}
...
To address the vanishing gradient problem, Hochreiter and Schmidhuber (1997) \ksremark{cytowanie tej pracy} proposed the long short-term memory (LSTM) network \cite{sharma2025review}. LSTMs incorporate a gating mechanism that allows memory cells to store information for longer durations and retrieve past computational results as needed \cite{sharma2025review}. The architecture employs three gates: an input gate controlling what information is stored, a forget gate determining what to discard, and an output gate regulating information passed to the next step \cite{sharma2025review}. Bidirectional LSTMs (BiLSTMs) process information in both directions, which is particularly valuable for identifying aspects and entities contributing to overall sentiment \cite{sharma2025review}.
...
Tang et al. \ksremark{cytowanie tej pracy} introduced neural network models combining convolutional and recurrent architectures for document-level sentiment classification \cite{behdenna2018document}. Their approach first learns sentence representations using either CNN or LSTM, then encodes the semantics and relations using gated RNNs \cite{behdenna2018document}. Experiments on Internet Movie Database (IMDB) and Yelp datasets showed that LSTM outperforms multi-filtered CNN for sentence representation \cite{behdenna2018document}.

\subsubsection{Transformer Architecture and BERT}
...
BERT leverages a self-attention mechanism to capture relationships between words within a sentence \cite{sharma2025review}. It follows a two-step approach: pretraining on massive text corpora using masked language modelling and next-sentence prediction, followed by fine-tuning on downstream tasks such as sentiment analysis \cite{sharma2025review}. BERT can tokenize text into subword units, handling various linguistic complexities, and has set new benchmarks across numerous NLP domains \cite{sharma2025review}.
...
\subsubsection{Emerging Architectures: LLMs and GNNs}
...
\begin{itemize}
	\item {Key strengths} include automatic feature learning, superior accuracy on large datasets, and versatility across tasks from basic polarity classification to aspect-based analysis \cite{sharma2025review}. Models can capture intricate relationships and adapt to new data, producing robust and generalizable systems \cite{sharma2025review}.
	
	\item {However}, these advantages come with substantial computational costs. Deep learning models typically require GPUs for training and inference, have large memory footprints, and are slower than classic models, making them less suitable for real-time applications and deployment on resource-constrained devices \cite{sharma2025review}.
	
	\item {Additionally}, optimal algorithm choice depends on the specific application and dataset: RNN variants (LSTM, GRU) excel with long-term dependencies, while CNNs offer efficiency for local pattern detection \cite{sharma2025review}. Recent research suggests RNNs are surpassing CNNs for many sentiment analysis tasks, particularly when long-range dependencies are involved \cite{sharma2025review}.
\end{itemize}

\ksremark{Niepokoi mnie, że ten rozdział jest praktycnie oparty na jednym źródle \cite{sharma2025review}.}

\section{Granular Computing}

Granular Computing (GrC) is a paradigm that processes complex information by treating data as aggregations known as information granules \cite{bargiela2003granular}. These granules emerge from abstracting raw data into knowledge and appear in diverse forms: time intervals (hours, days, weeks), image regions where clusters of pixels form recognisable objects, and words in natural language all function as granules \cite{bargiela2003granular}. Because human cognitive capacity is limited, people naturally partition complex information into simpler blocks based on shared properties, and each such block constitutes a granule \cite{qui2006GrC}. GrC therefore exploits the human ability for problem-solving by operating at multiple levels of abstraction \cite{qui2006GrC}.

Information Granulation (IG) is the process of constructing information granules \cite{bargiela2003granular}. An information granule is formally defined as a collection of elements grouped by indistinguishability, similarity, or functional proximity \cite{Liping2009GrC}. Granules can be arranged hierarchically: coarse granules provide a high-level overview, while finer granules reveal detailed information \cite{Liping2009GrC}. Three fundamental concepts underpin GrC, as originally articulated by Zadeh \cite{Zadeh1997GrC}:
\begin{itemize}
	\item \textbf{Granulation}: decomposition of the whole into parts,
	\item \textbf{Organisation}: integration of parts into the whole,
	\item \textbf{Causation}: association of causes and effects.
\end{itemize}

Concrete mechanisms for granulation are often seen in computing and human reasoning \cite{bargiela2003granular}. In computer systems, memory resources are granulated by adopting memory pages as basic operational chunks; swapping techniques then enable efficient access to individual data items \cite{bargiela2003granular}. When describing a problem, people tend to avoid numbers, instead using aggregates and constructing if-then rules that operate on those aggregates \cite{bargiela2003granular}. Although the physical world is inherently analog, computers perform digital processing, and this digitisation-grounded in set theory and interval analysis-exemplifies information granulation \cite{bargiela2003granular}. Furthermore, all data compression mechanisms represent instances of information granulation \cite{bargiela2003granular}.

\subsection{Three-Way Decisions}

Three-way decision (3WD) is a theory of thinking, problem solving, and information processing that relies on the use of three parts, three elements, or three levels \cite{Yao2018threeway}. It extends the classical binary (two‑way) approach-typically acceptance or rejection-by introducing a third option: non‑commitment, deferment, or uncertainty \cite{Fujita2016threeway, Yao2018threeway}. In many real‑world situations, making an immediate decision is unwise when evidence is insufficient; three‑way decision provides a principled way to withhold judgment until more information can be obtained \cite{Yao2018threeway}.
...
A general model of three‑way decision is the TAO (Trisecting–Acting–Outcome) framework \cite{Yao2018threeway}:
\begin{itemize}
...
\subsubsection{Data-Level Granulation: Three-Way Feature Selection and Granular Deep Learning}
...
\subsection{Sequential and Three-Way Decision Strategies}

In dynamic environments where data streams arrive over time, static sentiment classifiers become outdated~\cite{Yang2022temporal}. Yang et al. propose a temporal-spatial three-way granular computing framework that incrementally updates the model as new data arrives~\cite{Yang2022temporal}. At each time step, the model classifies samples into three regions: positive, negative, and boundary. Confident samples are accepted or rejected immediately, while uncertain ones are deferred to the next level, where finer granularity (more data and updated parameters) enables more reliable decisions~\cite{Yang2022temporal}. This sequential three-way strategy reduces misclassification and time costs without sacrificing accuracy, as shown on IMDB, Yelp, and Amazon datasets using TextCNN, TextRNN, TextRCNN, and fastText~\cite{Yang2022temporal}.

For open topic classification where test data may contain unseen classes, the same authors propose Three-Way Multi-Granularity learning towards Open topic classification (TWMG-Open)~\cite{Yang2022threeway}. The model operates at three granular levels: first, known topics are separated from unknowns using adaptive spherical decision boundaries; second, unknown samples are clustered via three-way DBSCAN to discover hidden classes; third, high-confidence clusters are added to a knowledge base for continual learning~\cite{Yang2022threeway}. Experiments on Banking77, StackOverflow, and TC20 show that TWMG-Open outperforms traditional baselines, especially when the proportion of known classes is high~\cite{Yang2022threeway}.

\subsection{Bag-of-Concepts Approaches}

Moving beyond word-level representations, several authors have explored \textit{bag-of-concepts} (BoC) approaches that aggregate semantically related tokens into higher-level information granules~\cite{Capillo2020mining, DeSantis2023prototype, Possemato2013AutomaticTC}. DeSantis et al.~\cite{DeSantis2023prototype} directly apply prototype theory: word embeddings are clustered into Voronoi regions, and each document is represented by a histogram of cluster memberships. This concept-level representation can rival word-level methods while offering better interpretability~\cite{DeSantis2023prototype}.

Capillo et al.~\cite{Capillo2020mining} present a granular computing framework that mines \(m\)-grams (short phrases) as information granules for text categorization. Each word in an $m$-gram is mapped to a vector using fastText embeddings, and the resulting sequence of vectors is compared using a custom edit distance that operates on word vectors rather than characters~\cite{Capillo2020mining}. A per-class clustering procedure extracts representative $m$-grams, which serve as alphabet symbols (concepts)~\cite{Capillo2020mining}. Documents are then transformed into symbolic histograms counting the occurrences of these concepts, and the histograms are classified by a SVM~\cite{Capillo2020mining}. A genetic algorithm optimises the system’s hyperparameters and selects the most discriminative symbols~\cite{Capillo2020mining}. Experiments on scientific abstracts (four classes) achieve about \(93\%\) accuracy, and on an SMS spam dataset about \(95\%\) accuracy~\cite{Capillo2020mining}. Importantly, the selected symbols are interpretable phrases (e.g., “string field theory” for theoretical physics, “customer care” for spam), demonstrating that the bag-of-concepts approach yields both high performance and explainable models~\cite{Capillo2020mining}.

Possemato et al.~\cite{Possemato2013AutomaticTC} extend the bag-of-concepts approach to handle unbalanced datasets by extracting $n$-grams and then running a per-class granular mining algorithm (RL-GRADIS) to obtain representative symbols for each category~\cite{Possemato2013AutomaticTC}. The union of these per-class symbols forms the final alphabet, and documents are converted into symbolic histograms by counting occurrences of each symbol~\cite{Possemato2013AutomaticTC}. A support vector machine with a Gaussian kernel then classifies the histograms, and a genetic algorithm performs wrapper-based feature selection to retain only discriminative symbols~\cite{Possemato2013AutomaticTC}. Experiments on the unbalanced Reuters-21578 dataset (ten categories) show that the per-class strategy improves macro-averaged F1 compared to the standard unsupervised version, demonstrating better handling of sparse classes~\cite{Possemato2013AutomaticTC}. The extracted symbols remain interpretable, such as phrases characterising the “Trade” class~\cite{Possemato2013AutomaticTC}.

\subsection{Interpretable Fuzzy Rule-Based Methods}

Interpretability is a key motivation for granular computing in sentiment analysis~\cite{Liu2017fuzzy}. Liu and Cocea propose a fuzzy rule-based system that replaces the traditional bag-of-words approach with a multi-granularity decomposition of text into sections, paragraphs, sentences, phrases, and words~\cite{Liu2017fuzzy}. This hierarchical granulation reduces instance complexity and dimensionality while enabling the extraction of local features at each level~\cite{Liu2017fuzzy}. Fuzzy logic handles linguistic uncertainty by assigning continuous membership degrees, reducing classification bias in subjective tasks~\cite{Liu2017fuzzy}. The resulting fuzzy rules are expressed in natural language, making the model fully transparent and interpretable~\cite{Liu2017fuzzy}. Experiments on movie reviews show competitive accuracy, though dimensionality remains high even after filtering~\cite{Liu2017fuzzy}.

\subsection{Multi-Granular Hierarchical Models}

\cite{Huang2022multigranular} propose a multi-granular joint sentiment-topic model (MgJST) that extends traditional LDA-based sentiment topic models by introducing two additional layers: a sentence layer between documents and words, and a sentiment layer between documents and topics~\cite{Huang2022multigranular}. Unlike previous models such as JST, which treat a document as a single topic mixture, MgJST assigns a distinct topic distribution to each sentence within a document, capturing finer-grained structural knowledge~\cite{Huang2022multigranular}. The model is a five-layer hierarchical architecture that uses Gibbs sampling for parameter estimation and can determine document-level sentiment polarity via a voting rule~\cite{Huang2022multigranular}. Experiments on seven review datasets show that MgJST significantly outperforms unsupervised baselines (WSTM and STSM) in sentiment detection F1 and achieves competitive performance with supervised deep learning methods~\cite{Huang2022multigranular}. The extracted topics are interpretable (e.g., positive topics for music contain “kiss the rain”, negative topics for electronics mention “sound quality” issues), demonstrating the model's ability to reveal opinionated topics at multiple granularities~\cite{Huang2022multigranular}.

In a related multi-granular approach for targeted sentiment analysis, \cite{Li2020GrC} propose Granular Computing-based Multi-level Interactive Attention networks (GC-MIA). Each word is treated as an indivisible granule, and a three-level interactive attention mechanism refines the representations of both the target and its context~\cite{Li2020GrC}. The first level uses matrix-based interactive attention to capture pairwise word interactions; the second level incorporates Euclidean distance to encode semantic closeness; the third level applies interactive attention between child granules (individual words) and parent granules (the aggregated target or context) to assign sentiment-relevant weights~\cite{Li2020GrC}. The final context and target granules are merged into an ancestor granule and fed to a softmax classifier~\cite{Li2020GrC}. On the SemEval 2014 laptop and restaurant datasets, GC-MIA achieves accuracies of 73.7\% and 79.0\%, respectively, outperforming LSTM, TD-LSTM, and the original interactive attention networks (IAN)~\cite{Li2020GrC}. A case study further visualises that the model assigns higher attention weights to context words semantically or sentimentally relevant to a given target, illustrating the explanatory power of the multi-level granular design~\cite{Li2020GrC}.

\section{Gap Analysis and Contribution}

The preceding sections have surveyed sentiment analysis at three levels (document, sentence, aspect) and highlighted persistent challenges: contextual polarity ambiguity, sarcasm and irony, negation handling, implicit opinions, and vocabulary evolution \cite{behdenna2018document, sharma2025review}. Existing methods fall into three broad families: lexicon-based (interpretable but brittle), classic machine learning (efficient but feature-sensitive), and deep learning (accurate but computationally expensive). Granular computing offers a conceptual bridge, but existing GrC work for text has not fully exploited the trade-offs within sentiment analysis.
...
\subsubsection{Gap 2 -- Bag-of-concepts via n-gram mining and clustering has been validated for topic and spam classification but not systematically evaluated for sentiment analysis.}
