...
\begin{enumerate}
	...
	\item \textbf{Uncertainty delegation:} A threshold $\thresh \in [0, 0.5]$ is defined. The intervals $[0, \thresh)$ and $(1-\thresh, 1]$ correspond to high-confidence negative and positive predictions, respectively, which are accepted without further processing. If the base model's probability lies within the closed interval
	\begin{equation}
		\thresh \le \probSmall{\mBase} \le 1 - \thresh,
	\end{equation}
	the prediction is considered unreliable and $\inputtext$ is placed in the boundary region.
	...
	\begin{itemize}
		...
		\item an SVM specialist trained on the same boundary samples to verify whether a two-stage traditional approach would work.
	\end{itemize}
\end{enumerate}
...
The cascade draws upon a collection of models that serve either as the fast base classifier or as the deep specialist. This subsection describes their architectures, training procedures, and the adjustments made for the cascade setting.

\subsubsection{Traditional Baselines}
\label{sec:trad_models}

A linear SVM and \logreg{} are employed as the computationally efficient baselines. Both are trained on the TF-IDF weighted $n$-gram vectors produced by the feature engineering pipeline (BoW representations are also evaluated during experimentation, but the final configuration converges on TF-IDF). The training utilizes the \code{scikit-learn} implementations with the following settings. Both models are fitted on the same training split and evaluated on the same validation and test sets.

\begin{itemize}
	\item \textbf{Linear SVM} optimizes the squared hinge loss with $L2$ regularisation. The default penalty $C=1.0$ is retained, and the solver is allowed up to $10\,000$ iterations to guarantee convergence on high-dimensional data. Because the cascade requires, a Platt scaling calibrator (\texttt{CalibratedClassifierCV}) is used. This post-processor maps the unbounded decision values to the $[0,1]$ interval and yields well-calibrated posterior probabilities for the positive class.
	
	\item \textbf{\logreg} directly models $\probLarge{}{\inputtext}$ by minimizing the log-loss (cross-entropy) with an $L2$ penalty. It is configured with the \code{L-BFGS} solver, $C=1.0$, and a maximum of $1\,000$ iterations. Unlike the SVM, \logreg{} natively outputs probability estimates without an external calibration step.
\end{itemize}

\subsubsection{Deep Learning Models}

The deep learning component of the cascade is based on DistilBERT, a distilled six-layer variant of BERT (\code{distilbert-base-uncased-finetuned-sst-2-english}) that retains most of the original model's performance while being substantially smaller and faster. It is already pre-trained on large English corpora and fine‑tuned on the Stanford Sentiment Treebank (SST‑2) and is available on the Hugging Face hub.

\begin{itemize}
	\item \textbf{Basic Fine-Tuning:} In this study, the model is further fine-tuned on the full IMDb training. Input texts (\code{text\_clean}) are tokenized with the DistilBERT tokenizer, truncated or padded to a maximum length of $256$ tokens. Training is performed for $3$ epochs with a batch size of $16$, using the Adam optimizer with a learning rate of $2 \times 10^{-5}$. Mixed-precision (\code{float16}) computation is utilized to reduce memory consumption and accelerate training. Early stopping monitors the validation loss (patience $2$ epochs), and a learning rate reduction on plateau (factor $0.5$, patience $1$) is applied. A stratified $20\%$ hold-out from the training set serves as the validation partition. After training, the model outputs class probabilities through a softmax over the two logits.
	
	\item \textbf{Specialist Fine-Tuning:} For the cascade ensemble, the basic DistilBERT model is further fine-tuned on the subset of validation instances that the base model could not classify with high confidence (the boundary region). The backbone is partially frozen: the first $5$ transformer layers are kept frozen, while the remaining layers and the classification head remain trainable. The specialist is trained for $10$ epochs with a lower learning rate of $2 \times 10^{-6}$, batch size $16$, and early stopping (patience $3$ epochs). All other training settings (mixed precision, tokenizer, maximum sequence length) are identical to the basic fine-tuning. This procedure encourages the model to specialize in the ambiguous region of the decision boundary without over-fitting to the relatively small hard-sample set.
\end{itemize}


\subsubsection{Specialist Training for the Cascade}
\label{sec:specialist_training}

In addition to the DistilBERT specialist, a traditional SVM specialist is constructed to evaluate whether a two-stage classical approach can boost performance on the boundary region. Both specialists are trained exclusively on the validation documents whose base model prediction probability $\probSmall{\mBase}^{(\idx)}$ falls within $[\thresh, 1-\thresh]$.

\begin{itemize}
	\item \textbf{SVM Specialist:} The same linear SVM architecture and hyperparameters as the base model are employed, but the model is fitted solely on the TF-IDF representations of the boundary samples. This yields a lightweight specialist that shares the feature space and decision function type of the base model while concentrating on the ambiguous region.
	
	\item \textbf{BERT Specialist:} The fine-tuning procedure is applied to the same boundary subset. Because the boundary region naturally contains an approximately equal number of positive and negative examples (a consequence of symmetric thresholding), no additional data balancing is required.
\end{itemize}
