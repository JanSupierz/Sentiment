...
\]
\ksremark{Wzory wycentrowane między akapitami zawsze numerujemy. Oczywiście \LaTeX\ robi to automatycznie:}
\begin{equation}
	(e_i, a_{ij}, s_{ijkl}, h_k, t_l),
\end{equation}
where:
\begin{itemize}
	\item $e_i$ is the name of an \ksremark{$i$-th?} entity (e.g., a product, service, organization, or individual),
	\item $a_{ij}$ is an \ksremark{$j$-th?} aspect of $e_i$ (a specific feature or attribute of that entity),
	\item $s_{ijkl}$ is the sentiment expressed toward aspect $a_{ij}$ of entity $e_i$, \ksremark{Co tutaj oznaczają $k$ i $l$?}
	\item $h_k$ is the \ksremark{$k$-th?} opinion holder (the person or organization expressing the opinion), and
...
\ksremark{W tytule sekcji damy pełną wersję zamiast skrótu PMI.}
...
\ksremark{w tytule podrozdziału bez skrótu}
Convolutional neural networks \ksremark{(CNN)}, originally developed for computer vision, were successfully adapted to natural language processing tasks by Collobert and Weston in 2008 \cite{sharma2025review}. A CNN architecture for text typically consists of three main layers: an input layer that receives tokenized text, a feature extraction layer comprising convolutional and pooling operations, and a classification layer with fully connected networks \cite{sharma2025review}.
...
Tang et al. \ksremark{cytowanie tej pracy} introduced neural network models combining convolutional and recurrent architectures for document-level sentiment classification \cite{behdenna2018document}. Their approach first learns sentence representations using either CNN or LSTM, then encodes the semantics and relations using gated RNNs \cite{behdenna2018document}. Experiments on IMDB \ksremark{Trzeba rozwinąć skrót przy pierwszym jego wystąpieniu.} and Yelp datasets showed that LSTM outperforms multi-filtered CNN for sentence representation \cite{behdenna2018document}.

\subsubsection{Transformer Architecture and BERT}

The most significant breakthrough in deep learning for NLP came with the transformer architecture and its pretrained variants \cite{vaswani2017attention}. BERT (Bidirectional Encoder Representations from Transformers), developed by Google in 2018 \ksremark{cytowanie}, revolutionized language understanding through its bidirectional architecture that considers both preceding and following words in context \cite{sharma2025review}.

BERT leverages a self-attention mechanism to capture relationships between words within a sentence \cite{sharma2025review}. It follows a two-step approach: pretraining on massive text corpora using masked language modelling and next-sentence prediction, followed by fine-tuning on downstream tasks such as sentiment analysis \cite{sharma2025review}. BERT can tokenize text into subword units, handling various linguistic complexities, and has set new benchmarks across numerous NLP \ksremark{rozwinięcie skrótu} domains \cite{sharma2025review}.
