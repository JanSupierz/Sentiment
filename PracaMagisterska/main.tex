...
To address the sparsity inherent in BoW and TF‑IDF representations, an optional clustering pipeline is implemented. Dense sentence embeddings for the extracted $n$-grams are generated using the Sentence Transformer model \texttt{all-MiniLM-L6-v2}. These embeddings can be concatenated with VADER sentiment scores, weighted by a tunable hyperparameter, to enforce a sentiment‑aware distribution. Following $L_2$ normalization, Facebook AI Similarity Search (FAISS) is utilized to perform K‑Means clustering. This groups semantically similar $n$-grams into $\kconcepts$ discrete concepts, compressing the feature space while retaining semantic granularity.
...
\begin{itemize}
	\item \textbf{Traditional Baselines:} a linear SVM and \logreg are employed. Because the cascade architecture relies on predictive certainty, the SVM---which outputs uncalibrated distances to the separating hyperplane---is wrapped in a Platt scaling calibrator (\texttt{CalibratedClassifierCV}) to yield well‑calibrated probabilistic outputs.
	\item \textbf{Deep Learning Classifier:} DistilBERT, a distilled, computationally efficient variant of the BERT architecture, is utilized. This model is fine‑tuned using mixed‑precision training, early stopping, and learning rate reduction on plateaus to prevent overfitting. (\texttt{distilbert-base-uncased-finetuned-sst-2-english})
\end{itemize}

\section{Cascade Ensemble}

To balance the computational efficiency of traditional machine learning algorithms with the deep understanding capacity of transformer‑based models, a cascade ensemble architecture is proposed. The system routes instances dynamically according to the predictive certainty of a lightweight base model.

\begin{enumerate}
	\item \textbf{Initial inference:} A computationally inexpensive base model evaluates an input text $\inputtext$ and outputs a probability score $P(y=1 \mid \inputtext)_{\text{base}}$.
	
	\item \textbf{Uncertainty delegation:} A threshold $t_{\text{lower}}$ is defined. If the base model's probability falls within the uncertain interval $t_{\text{lower}} \le P(y=1 \mid \inputtext)_{\text{base}} \le 1 - t_{\text{lower}}$, the prediction is considered unreliable and is deferred. The threshold is chosen by optimizing the trade‑off between coverage and accuracy on the validation set.
...
In this section, it is being evaluated what parameters have impact on the robustness of sentiment analysis. Finally, a cascade ensemble approach is evaluated.

... Jeszcze nie gotowe

Impact of distinct phases is evaluated...
