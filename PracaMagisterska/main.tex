...
\begin{enumerate}
	...
	\item \textbf{Typographical unification} uses a translation table to replace uncommon characters with their ASCII equivalents.
...
\begin{itemize}
	\item \textbf{Auxiliary verbs (except modals)}: tokens with the coarse POS tag \texttt{AUX} are removed, unless their fine-grained tag is \texttt{MD} (modal auxiliary, e.g., \textit{can}, \textit{will}, \textit{must}).
...
\subsubsection{Custom Vectorization}
...
\subsubsection{Statistical Pruning}
