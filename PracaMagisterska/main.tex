...
Sentiment analysis methods can be broadly divided into two main categories: traditional approaches and machine learning approaches \cite{sharma2025review}. Within machine learning, a further distinction exists between classic supervised approaches and modern deep learning techniques \cite{behdenna2018document}.
...
\subsubsection{Semantic Orientation and PMI}
...
\subsubsection{Major Sentiment Lexicons}
...
However, misclassifications occurred due to several factors: limited vocabulary coverage, difficulty detecting sarcasm, word ambiguity, and challenges with misspelled or abbreviated terms \cite{nursal2025battle}. For instance, VADER misclassified reviews containing sarcasm or domain-specific expressions, highlighting the inherent limitations of pure lexicon-based approaches \cite{nursal2025battle}.
...
\item For Arabic sentiment analysis, Rushdi Saleh et al. introduced the OCA corpus of movie reviews and evaluated NB and SVM classifiers \cite{behdenna2018document}. In the Chinese domain, NB was found to outperform SVM for mobile review classification \cite{behdenna2018document}. 
...
Features suitable for document and sentence level analysis are typically target-independent and thus inapplicable to aspect-level tasks, where the core objective is identifying opinion targets \cite{azzeddine2018sentiment}. Aspect-level supervised learning therefore requires either target-dependent feature generation or mechanisms to determine the application scope of sentiments that cover specific entities or aspects within a sentence \cite{azzeddine2018sentiment}. Additionally, aspect-level supervised learning demands annotated datasets containing labeled aspects and non-aspects, which are more costly to produce than document-level polarity labels \cite{azzeddine2018sentiment}.

\subsection{Deep Learning for Sentiment Analysis}

Deep learning has revolutionized sentiment analysis by offering several advantages over traditional machine learning approaches \cite{sharma2025review}. Unlike methods that rely on manually crafted features, deep learning models can automatically discover and learn relevant representations directly from the data, eliminating time-consuming and potentially error-prone feature engineering \cite{sharma2025review}. This automatic feature learning capability is particularly beneficial for large-scale sentiment analysis tasks involving social media posts or customer reviews, where deep learning algorithms excel at processing vast amounts of data due to their parallel processing capabilities \cite{sharma2025review}.

Studies have consistently shown that deep learning models achieve superior accuracy compared to traditional methods such as SVM and Naïve Bayes \cite{behdenna2018document, sharma2025review}. Their ability to learn intricate relationships and adapt to new data leads to more robust and generalizable models that perform well on unseen data \cite{sharma2025review}. Furthermore, deep learning offers versatility across various sentiment analysis tasks, from basic polarity classification to aspect-based analysis and emotion detection \cite{sharma2025review}.
...
\subsubsection{Long Short-Term Memory (LSTM) and Gated Recurrent Unit (GRU)}
...
Tang et al. introduced neural network models combining convolutional and recurrent architectures for document-level sentiment classification \cite{behdenna2018document}. Their approach first learns sentence representations using either CNN or LSTM, then encodes the semantics and relations using gated recurrent neural networks \cite{behdenna2018document}. Experiments on IMDB and Yelp datasets showed that LSTM outperforms multi-filtered CNN for sentence representation \cite{behdenna2018document}.
